async function loadData() {
  ...
  const res = await fetch(`${dataPath}?v=20260521y`, { cache: "no-store" });
...
function homePathCards(data, daily) {
  ...
  const rows = seeds.slice(0, 4).map(seed => {
    ...
    return `<article class="path-card">
      ...
      <div>${links.map(b => `<a href="pages/brand-artemio.html?brand=${encodeURIComponent(b.slug)}">${b.name}</a>`).join("")}</div>
    </article>`;
  });
  return rows.join("");
}
