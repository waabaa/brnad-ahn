...
// ─── 5) 산업별 카드 페이지 ────────────────────────────────────────────────
// pinned 산업(AI)은 넓은 카드로 맨 앞에 둔다. 폭은 나머지 카드 수에 맞춰 정한다 — 데스크톱 4열에서 빈칸이 남지
// 않도록(나머지 14개면 2칸, 12개면 한 줄 전체). 900px 미만에서는 일반 카드와 같다(좁은 격자에서 span 이 암묵 열을 만든다).
// 스타일은 페이지 안 <style> 로 둔다(styles.css 를 바꾸면 CSS_V 를 올려 전 페이지를 다시 써야 한다).
const pinnedSpan = (() => { const rem = industryGroups.filter(([s]) => !PINNED.has(s)).length % 4; return rem === 0 ? "1/-1" : `span ${4 - rem}`; })();
const PINNED_STYLE = `<style>@media (min-width:900px){.cat-card.pinned{grid-column:${pinnedSpan}}.cat-card.pinned .logos{grid-template-columns:repeat(10,1fr)}}</style>`;
// 로고 타일은 로고가 있는 브랜드부터 채운다 — 점수순 상위에 로고 없는 항목이 끼면 빈 칸이 줄지어 보인다.
const logoFirst = (a, b) => (hasLogo(b) - hasLogo(a)) || byScore(a, b);
function catCard(slug, g, prefix = P) {
  const pinned = PINNED.has(slug);
  const top = [...g.items].sort(logoFirst).slice(0, pinned ? 10 : 5);
  return `<a class="cat-card${pinned ? " pinned" : ""}" href="${prefix}category/${slug}.html"><span class="head"><b>${esc(g.label)}</b><span class="n">${g.items.length}개</span></span><p>${esc(pinned ? (INDUSTRY_DESC[slug] || "") : oneLineIndustry(slug))}</p><span class="logos">${top.map(b => `<span>${hasLogo(b) ? `<img src="${assetHref(b.logo, prefix)}" alt="" loading="lazy" decoding="async">` : ""}</span>`).join("")}</span></a>`;
}
function oneLineIndustry(slug) { return (INDUSTRY_DESC[slug] || "").replace(/\s+/g, " ").slice(0, 70); }
{
  ...
  const body = `${hubHead("산업별 브랜드", "산업별로 찾는 브랜드", esc(`브랜드가 실제로 경쟁하는 시장을 기준으로 ${industryGroups.length}개 산업으로 나눴습니다. 카드를 누르면 해당 산업의 전체 브랜드 목록으로 이동합니다.`), crumbsHub("산업별"))}<div class="wrap section">${PINNED_STYLE}<div class="cat-grid">${cards}</div></div><div class="wrap">${blocks}</div>`;
