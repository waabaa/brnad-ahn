...
**8단계 — 네이버 이미지 검색(Playwright 실브라우저) + 전건 육안 검증**: 나무위키에 문서가 없거나 플레이스홀더만 있던 나머지 브랜드를 대상으로 네이버 이미지 검색을 실제 브라우저(Playwright)로 렌더링해 수집했다. 정적 HTML curl 스크래핑은 검색어와 무관한 콘텐츠만 반환해(JS 렌더링 필요) 폐기하고, 렌더링된 DOM에서 `img` 태그의 `alt` 텍스트에 "로고/CI/BI/심볼/마크/엠블럼" 키워드가 있는 후보만 1차로 걸렀다. 이후 **모든 후보를 다운로드해 실제로 열어보고 육안으로 확인**한 것만 반영했다 — alt 텍스트 매칭만으로는 부족하다는 게 실제로 확인됐다(예: "안흥찐빵 로고"라는 alt를 단 이미지가 실제로는 "밀원"이라는 다른 하위 브랜드 로고, "신원 로고" 검색 결과가 동명이인 치과 회사 "Shinwon Dental", "횡성한우협동조합" 검색 결과가 실제로는 지역이 다른 "전북한우협동조합", "불스원" 검색 결과가 레드불과의 상표분쟁 비교 기사 사진이었던 사례 등). 이렇게 걸러내고 확정한 **19건**을 추가 반영했다(행복나래·해법수학·두끼떡볶이·놀부·쌍방울·공정무역가게·종로학원·네이버쇼핑·한솔교육·튼튼영어·세정·어퓨·신원·한라산소주·홍성한우·유디치과·신일전자·핀다·처갓집양념치킨).

**최종 결과: 300건 중 269건(89.7%)에 로고 확보 — 검증됨 197건 + 비공식/연구용 72건. 31건은 여전히 미확보.**

- 로고를 판정할 때 UI/공개 배포 단계에서는 `logoSourceTier`를 반드시 확인해 비공식 로고를 공식 브랜드 자산처럼 노출하지 않아야 한다(워터마크·출처 표기 또는 별도 섹션 분리 권장).
- 남은 31건(타다·알맹상점·GS더프레시·ST유니타스·공부의신·복음자리·불스원·에이블리·노리단·사회연대은행·오이뮤·강화섬쌀·횡성한우협동조합·봉평메밀·춘천닭갈비·핏펫·닥터도리·바잇미·어바웃펫·뽀로로파크·아가방앤컴퍼니·보령메디앙스·강남베드로병원·미소치과·한일전기·아트조선·제주도립미술관·잉크테크·내추럴플러스·CJ웰케어)는 8가지 방법(Commons API·공식홈페이지·WebSearch·위키백과·Wikidata·나무위키·네이버 이미지)을 모두 거쳤지만 검색될 때마다 동명이인·다른 지역·다른 하위 브랜드·디자인 공모전 습작 등 부정확한 결과만 나왔다. 목록은 `300-brands/logo-final-status.json`의 `stillMissingLogoIds` 참고. `reports/data-completion-backlog.csv` 우선순위에 반영해 리서처가 공식 홈페이지·보도자료·SNS 프로필 등에서 수동 수집해야 한다. **추측 매칭으로 채우지 말 것** — 검증 없이 자동 반영하면 위 사례처럼 틀린 브랜드의 로고가 들어간다.
