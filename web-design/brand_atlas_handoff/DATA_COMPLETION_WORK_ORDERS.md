...
**최종 결과: 300건 중 197건(65.7%)에 검증된 로고 확보, 103건 미확보.**

- 103건이 남은 이유는 네 가지다: (1) 도메인은 맞지만 사이트가 og:image/아이콘을 제공하지 않음, (2) 봇 차단(403)으로 확인 자체가 불가 — 넥슨·오리온의 실제 공식 도메인(nexon.com, orionworld.com)도 이 환경에서 fetch 자체가 차단되어 재시도 3회 후에도 실패, (3) Wikimedia Commons/위키백과/Wikidata/공식 홈페이지 어디에도 신뢰할 수 있는 로고 파일이 없음(주로 프랜차이즈·중소기업 — 제3자 스톡 이미지 사이트에만 존재), (4) 강화섬쌀·홍성한우 등 지역 협동조합류는 애초에 확인 가능한 공식 웹사이트 자체가 없을 가능성.
- **6가지 독립적 방법**을 모두 시도했다: (1) Commons API 검색, (2) 공식 홈페이지 og:image/파비콘 직접 fetch, (3) WebSearch 보조 Commons/위키백과 탐색, (4) site 제한 없는 광역 WebSearch, (5) 위키백과 인포박스 이미지(`pageimages` API), (6) Wikidata P154(로고 이미지 속성) 조회. 회차를 거듭할수록 적중률이 급감했다(76% → 29% → 7% → 13% → 0%/10 → 1%/103) — 자동화로 안전하고 정확하게 확보할 수 있는 한계에 도달했다.
- **이 6가지 결과가 수렴하는 결론**: 남은 103건은 "덜 찾아봐서"가 아니라 Wikimedia/Wikidata 생태계와 공식 홈페이지 어디에도 자유 라이선스·공식 로고 자산이 존재하지 않는 브랜드들이다. 이 이상 계속하면 pngegg/pngwing 같은 출처 불명 스톡 사이트 이미지를 끌어오게 되어 "정확성"이라는 원칙 자체를 훼손하게 된다 — 이 프로젝트의 원칙(추정 문장으로 채우지 않는다, 출처가 불명확하면 채우지 않는다)과 정면으로 배치된다.
- 목록은 `300-brands/logo-final-status.json`의 `stillMissingLogoIds` 참고. `reports/data-completion-backlog.csv` 우선순위에 반영해 리서처가 공식 홈페이지·보도자료·SNS 프로필 등에서 수동 수집해야 한다. **추측 매칭으로 채우지 말 것** — 이번 작업에서 여러 차례 확인했듯 동명이인·유사명·구 브랜드 오탐 위험이 실재하며, 방법을 바꿔도 매번 재발한다.
