...
**영문 근거 수록(`--source en`, 2026-09-12 사용자 승인).** 기준 목록(S&P 500·나스닥100 — `reports/index-coverage-*.json`)
기업 중 한국어 문서가 없거나 너무 짧은 개체는 영문 위키백과 본문 + 위키데이터 팩트를 근거로 수록한다. 숫자 검증·문체
규칙은 같고, 단위 환산(billion→억)은 근거 밖 숫자로 기각된다. 한글 표기는 위키데이터 ko 레이블이 있을 때만 쓰고,
없으면 원어만 쓴다(§1 음차 금지). `entityLinks.source`는 `wikidata en sitelink (import ...)`.
