...
## 2026-09-13 — 아틀라스 매거진 창간 · S&P 500·나스닥100 수록 마무리 · 광고 심사 대비 정리

- **아틀라스 매거진(`/magazine/`) 창간.** 브랜드 여러 곳을 한 주제로 엮는 기획 기사. 월간 호(No.01 = 9월호) + 매주 한 편,
  원고의 공개일이 되면 주간 리프레시가 자동 공개한다. 창간호 3편(AI 브랜드 아홉 곳의 아이덴티티, 로고 뒤의 스튜디오,
  사명 변경 16곳)을 열고 2편(9/21·9/28)은 예약. 원고의 숫자가 브랜드 데이터에 없으면 빌드가 멈춘다(`verifyArticle`).
  지면 디자인은 로고 도판(plate)과 실제 팔레트 색 띠가 시그니처(`magazine.css`). 홈·브랜드 페이지·sitemap·RSS·llms.txt에 연결.
- **S&P 500·나스닥100 489 / 513.** 영문 근거 수록 110건 추가(재시도 포함). 남은 6곳(Allegion·Blackstone·FirstEnergy·
  IDEX·Lowe's·Verisign)은 생성문의 숫자 검증을 두 번 통과하지 못해 보류. 지수 구성 기업은 한글 표기·로고가 없어도 목록에 올린다.
- **신규 로고 검수 180장 → 12건 제거**: 옛 사명 로고(Square→Block, Delphi→Aptiv, FLEETCOR→Corpay, BNY Mellon, The Linde Group,
  Smurfit Kappa, Sempra Energy, 합병 전 Zimmer, WASTE MANAGEMENT, Marsh & McLennan Companies), 다른 개체 로고(Vivmark에 붙은
  Equity Residential), 간판 사진(Cognizant).
- **독자에게 운영 용어를 보이지 않는다.** "디렉토리 등급 자료입니다. 본문은 수록 자료를 그대로 옮긴 것이며…" 안내문을 없애고,
  아카이브 수집분 463건의 운영 문장("메타데이터 중심으로 관리합니다" 등)을 구조화 값으로 다시 썼다(`clean-archive-boilerplate.mjs`).
  디렉토리 목록의 공개 명칭은 '원어 표기 브랜드'. 문장이 짧아진 24쪽이 본문 800자 미만이 되어 색인 제외로 바뀌었다(페이지는 유지).
- **공개 웹루트 정리**: 어느 페이지도 쓰지 않던 외부 매체 수집 자료(`archive/korean-sources`, 87MB)·초기 작업 원본·내부 문서를 배포에서
  제외하고, 디자인 시안 3쪽(brand-artemio·mobile·other-pages)을 지우고 301로 돌렸다.
- 인사이트 목록의 백과사전 발췌 2건(구찌·아디다스)을 브랜드 항목의 자체 문장으로 교체. 화장품 브랜드 7곳을 '헬스·제약'에서 '뷰티'로.
- 개인정보처리방침에 광고와 쿠키 조항 추가. 허브 제목 숫자를 쉼표 형식으로(1,902). 문의 API가 3일간 502였던 것을 복구하고
  서비스를 `Restart=always`로.

## 2026-09-12 (저녁) — S&P 500·나스닥100 1차 수록 · 금융/에너지·산업재 산업 · 색인 병기

- **S&P 500·나스닥100 커버리지 58 → 289 / 513**(`reports/index-coverage-2026-09-12.json`, 영문 위키백과 구성 종목표 →
  Wikidata QID). 한국어 문서가 있는 113곳은 기존 규칙으로, 없거나 짧은 곳은 **영문 근거 수록**(`--source en`, 사용자 승인)으로
  118곳. 나머지 224곳은 준비 캐시에 있어 게이트웨이 한도가 풀리는 대로 이어서 수록한다.
  영문 근거 레코드는 한글 표기를 위키데이터 ko 레이블이 있을 때만 쓴다(음차 금지).
- **산업 2개 추가 — 금융(121), 에너지·산업재(99)**(`scripts/apply-index-sectors.mjs`). 지수 기업은 공식 섹터(GICS·ICB)로,
  기존 브랜드는 은행·핀테크·에너지 컬렉션(Wikidata 근거)으로 옮겼다. 운송 하위산업(항공·물류·철도)과 소비자 분류에 있던
  브랜드는 섹터로 옮기지 않는다 — 섹터는 투자 분류라 브랜드 경험을 반영하지 않는다. 정유사가 '모빌리티'에 있던 것도 바로잡혔다.
- **가나다·ABC 색인 병기** — 한글 표기가 있는 브랜드는 초성 색인에만 있어 A–Z에서 OpenAI·Apple이 보이지 않았다. 양쪽에 싣는다.
- 게이트웨이 기본 upstream 타임아웃(약 30초)이 두 건 묶음 생성을 잘라 대부분 실패하던 문제 — 요청에 `timeout_ms` 170초 명시.
- 신규 로고 191건 육안 검수, 옛 사명 로고(Gen Digital=NortonLifeLock, RTX=Raytheon, Strategy=MicroStrategy,
  Elevance=WellPoint, Expand=Chesapeake, APA=Apache)·하우스 플래그(Carnival) 등 8건 제거.
